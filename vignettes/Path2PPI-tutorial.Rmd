...
organism. To overcome these drawbacks **Path2PPI** predicts a certain pathway 
of interest in one organism by using and combining the PPIs of this pathway 
in other well established model organisms.
...
generate and prepare the necessary data files, accordingly.
...
showInteraction(ppi,interaction=c("B2AT71","B2AE79"),mode="detailed",
                verbose=FALSE)
```
This data frame contains each single predicted interactions of the current 
...
publication) and from which data base the interaction of the proteins "P42345" 
...
**Aim:**
We aim to predict a pathway-specific interaction network in the target
organism based on the interaction networks of well established model 
organisms. We consider the degree of homologies and the number of reference 
species.
...
bound $h_u$ and the lower bound $h_l$. During the algorithm, the range is 
required to map the BLAST E-value interval $[h_l ,h_u]$ to a homology score in 
the interval $[1,0]$ where 0 is the worst and 1 the best value. 
...
**Figure 1. Steps of the prediction algorithm** a) Searches for 
...
homologous protein from the first set 
($H_a$) will now be connected with each homologous protein from the other set 
($H_b$). The deduced interactions are scored, using the function described 
below. b) The linear function to map the BLAST E-values to the interval 
$[0,1]$. c) Combining the redundantly predicted interactions to one final PPI 
network, $PPI_p$, of the target species.  
...
$V_H \subseteq (V_h \cup V_y \cup V_p)$
...
Before starting the algorithm we have to set an upper and lower bound for 
the E-values to define a range $[h_l,h_u]$ which will be mapped to the 
interval $[0,1]$ (figure 1b).
The PPIs of each reference species are handled separately. Each interaction 
of each reference species is handled one by one. We start with the first 
reference species (here, $PPI_h$):
...
(figure 1a, dotted edges to set $H_a$ or set $H_b$)

2. If $V(H_a)=\emptyset$ or $V(H_b)=\emptyset$, decline $e_h$, otherwise 
compute the homology score $s_h \to [0,1]$ for each edge $e_a \in H_a$ and 
each edge $e_b \in H_b$ based on the single E-values and the sizes of $H_a$ 
and $H_b$:  
...
4. Add each remaining node $\{v \in V(\{H_a \cup H_b\}) \wedge v \in V_p\}$ 
...
4. Combine the two intra species scores to one common inter species or final 
score $i_{a,b}$ (figure 1c lower area):  
